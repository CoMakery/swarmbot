/*
Edited from the original to use CommonJS and be usable in Node.
Should really do a fork on github.
~Duke
*/
...
Backbone.Firebase._checkId = function(snap, idAttribute) {
  ...
  if (Backbone.Firebase._isPrimitive(model)) {
    Backbone.Firebase._throwError(
      'InvalidIdException: Models must have an Id. Note: You may ' +
  ...
  SyncModel.protoype = {
    fetch: function(options) {
      Backbone.Firebase._promiseEvent({
        ...
        complete: function() {
          Backbone.Firebase._onCompleteCheck(this._initialSync.err,
            this, options);
        }
      });
    }
...
Backbone.Firebase.Model = Backbone.Model.extend({
  ...
  /**
   * Silently set the id of the model to the snapshot key
   */
  _setId: function(snap) {
    // if the item new set the name to the id
    if (this.isNew()) {
      this.set(this.idAttribute, Backbone.Firebase._getKey(snap), {
        silent: true
      });
    }
  },
...
var OnceCollection = (function() {
  ...
  OnceCollection.protoype = {
    ...
    create: function(model, options) {
      ...
      model[this.idAttribute] = model[this.idAttribute] || Backbone.Firebase
        ._getKey(this.firebase.push());
      options = _.extend({
        autoSync: false
      }, options);
      return Backbone.Collection.prototype.create.call(this, model,
        options);
    },
    /**
...
var SyncCollection = (function() {
  ...
  SyncCollection.prototype = {
    ...
    _updateModel: function(model) {
      ...
      updateAttributes = this._compareAttributes(remoteAttributes,
        localAttributes);
    ...
    _compareAttributes: function(remoteAttributes, localAttributes) {
      ...
      var union = _.union(_.keys(remoteAttributes), _.keys(
        localAttributes));
    ...
    _removeModel: function(model, collection, options) {
      ...
      Backbone.Firebase._setWithCheck(childRef, null, _.bind(options.success,
        model));
    },
  ...
  constructor: function(model, options) {
    ...
    this.model = function(attrs, opts) {
      ...
      newItem.on('change', function(model) {
        var updated = Backbone.Firebase.Model.prototype._updateModel(
          model);
        model.set(updated, {
          silent: true
        });
